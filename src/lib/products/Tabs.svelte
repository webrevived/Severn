...
<div class="relative flex justify-around pb-6" border="b-1px brown-1200" bind:this={container}>
	{#each options as item}
		<button on:click={onClick(item, this)} class="heading-3 text-lg font-medium text-black-600"
			>{item}</button
		>
	{/each}
